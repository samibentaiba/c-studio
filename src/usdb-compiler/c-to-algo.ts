...
export function translateCToAlgo(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const funcMatch = line.match(
      /^(int|float|double|char|bool|void)\s+(\w+)\s*\(([^)]*)\)\s*{?/
    );
    if (funcMatch && !inMain) {
      inFunction = true;
      currentFunctionReturnType = typeMap[funcMatch[1]] || "INTEGER";
    ...
    if (inMain || inFunction) {
      ...
      const varMatch = line.match(/^(int|float|double|char|bool)\s+(.+);$/);
      if (varMatch) {
...
function parseParams(
  ...
  for (const param of params) {
    const match = param.match(/^(int|float|double|char|bool)\s+(\w+)$/);
    if (match) {
      const algoType = typeMap[match[1]] || "INTEGER";
      result.push(`${match[2]} : ${algoType}`);
    }
  }
...
function parseVariableDeclaration(
  ...
  const algoType = typeMap[cType] || "INTEGER";
...
function translateStatement(
  ...
  const declMatch = line.match(/^(int|float|double|char|bool)\s+(.+)/);
  if (declMatch) {
    parseVariableDeclaration(
      declMatch[1],
      declMatch[2],
