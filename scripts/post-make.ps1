...
# The extracted content is in a subfolder
$ExtractedFolder = Get-ChildItem -Path $TempDir -Directory | Select-Object -First 1

# Create clean structure
Write-Host "Creating clean folder structure..." -ForegroundColor Green
New-Item -ItemType Directory -Path "$CleanDir\runtime" -Force | Out-Null

# Move all files to runtime folder
...
Get-ChildItem -Path $ExtractedFolder.FullName | ForEach-Object {
    Move-Item -Path $_.FullName -Destination "$CleanDir\runtime\" -Force
}
...
Compress-Archive -Path $CleanDir -DestinationPath $OriginalZip -Force
